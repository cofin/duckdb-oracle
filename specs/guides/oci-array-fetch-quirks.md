...
2. **Bind LOBs as Long Binary (`SQLT_LBI`/`SQLT_LNG`) or String (`SQLT_STR`)**
    - **Problem**: `SQLT_STR` binding for `BLOB` causes `ORA-00932` (incompatible type).
    - **Fix**: Use `SQLT_LBI` for `BLOB`.
    - **Refinement**: For `CLOB`, `SQLT_STR` is actually preferred (and used in final implementation) to ensure proper character encoding handling and avoid binary-to-text issues. This resolved `ORA-00932` for XMLTYPE (which is fetched as CLOB).
    - **Result**: `BLOB`/`RAW` use binary fetch. `CLOB`/`XMLTYPE` use string fetch.

3. **Handle RAW as String (`SQLT_STR`)**
    - **Problem**: `SQLT_BIN` binding for `RAW` caused corruption (shift).
    - **Workaround**: Bind `RAW` as `SQLT_STR`? (Re-verified: `RAW` uses `SQLT_BIN` in current code with `needs_server_conversion` check. If `try_native_lobs` is true, it uses `SQLT_BIN`).
    - **Current State**: `RAW` uses `RAWTOHEX` conversion in SQL if `needs_server_conversion` is true (which is false by default unless configured).

4. **Disable Specific Tests**
    - **Status**: `test_xml_blob` (RAW column) and `test_vectors` (VECTOR column) are now **FIXED** and passing.
    - **Fix**: `VECTOR` uses `VECTOR_SERIALIZE` (always). `XMLTYPE` (CLOB) uses `SQLT_STR` fetch.
