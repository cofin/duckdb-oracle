...
| **Numeric** | `SQLT_NUM` | `BIGINT`/`DOUBLE` | Scale 0 & precision ≤18 -> `BIGINT`, else `DOUBLE`. |
| **String** | `SQLT_CHR` | `VARCHAR` | |
| **Date/Time** | `SQLT_TIMESTAMP_TZ` | `TIMESTAMP` | |
| **LOBs** | `SQLT_CLOB`, `SQLT_BLOB` | `VARCHAR`/`BLOB` | |
| **Identifiers** | n/a | n/a | Schema/table identifiers are quoted to avoid case/injection issues. |
...
- **Helper**: We can add a scalar function `oracle_attach_wallet('path/to/wallet_dir')` which sets the internal environment or OCI context parameter for the session. The path must exist and be a directory; otherwise the call fails fast to surface misconfiguration.
