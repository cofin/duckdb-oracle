...
- [x] Commit changes
- [x] Verify CI passes (pending push)
- [ ] Archive workspace (pending CI pass)

### Phase 5: Connection Hang Remediation & Optimization ✅
...
- [x] Wire streaming fetch end-to-end (no full buffering):
  - [x] Bind: prepare/describe, store stmt/handles
  - [x] Init global: create `OracleScanState`, bind defines once to persistent buffers, hold svcctx/stmt/err
  - [x] Scan: execute cursor once, array-fetch STANDARD_VECTOR_SIZE, copy to chunk
- [x] Fix current hang and instability:
  - [x] Fixed "free(): invalid next size" crash (Array Fetch buffer sizing)
  - [x] Fixed `ORA-01007` (Duplicate columns in `GetScanFunction` vs `BindInternal`)
  - [x] Fixed `INTERNAL Error` (TransactionManager mismatch and Optional ptr dereference)
  - [x] Fixed PL/SQL test syntax
- [x] Validate locally: `make test` (Unit tests passed)
- [x] Validate locally: `make integration` (All integration tests passed)

## Progress Log

### 2025-11-23 - Implementation & Fixes
- Implemented `OracleConnectionManager` with pooling and timeouts to prevent hangs.
- Implemented `OCI_THREADED` mode.
- Implemented **Array Fetch** (Streaming) in `OracleQueryFunction` for performance.
- Fixed memory corruption bugs in buffer allocation.
- Fixed logic errors in `OracleTableEntry` causing duplicate columns and `ORA-01007`.
- Fixed internal DuckDB errors by overriding `GetCatalogVersion` and `OracleInitGlobal` placement.
- Validated all 4 complex integration test suites against real Oracle container.
